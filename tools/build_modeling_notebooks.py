...
def operation_length_supplementary_cells():
    """Extra operation-length checks requested during model improvement."""
    return [
        md("""
## 12. Operation-length improvement and supervisor checks

This section records the additional operation-length experiments requested after the main benchmark. These checks do not replace the leakage-safe benchmark above; they explain why some tempting changes were accepted, rejected, or kept as provisional.

The checks answer five practical questions:

1. Should `ExpectedDurationMins` be removed?
2. Does provisional `operation_start_hour` help?
3. Does the full procedure code, consultant, or first-letter chapter help enough to keep?
4. Does a log-transformed target help with skewed operation lengths?
5. Can CatBoost or a neural network beat XGBoost?

The extra result files are generated by `tools/experiment_operation_length_expected_duration_sensitivity.py` and `tools/experiment_operation_length_r2.py`. They are loaded here only if present, so the notebook remains readable even before the optional experiments are rerun.

**Where the supervisor can find the key result evidence:**

| Evidence | Notebook location | Click to open |
|---|---|---|
| Main model benchmark across algorithms | Section 7 | [algorithm_comparison.csv](../result/modeling/operation_length_mins/algorithm_comparison.csv) |
| Dummy and hospital-planned-duration benchmark | Section 10 | [operational_benchmarks.csv](../result/modeling/operation_length_mins/operational_benchmarks.csv) |
| Final untouched-test metrics | Section 9 | [final_test_metrics.csv](../result/modeling/operation_length_mins/final_test_metrics.csv) |
| ExpectedDurationMins with/without test | Section 12.1 | [expected_duration_sensitivity_results.csv](../result/modeling/operation_length_mins/expected_duration_sensitivity/expected_duration_sensitivity_results.csv) |
| Validation versus test plot | Section 12.1 | [open validation/test plot](../result/modeling/operation_length_mins/expected_duration_sensitivity/validation_test_comparison.png) |
| Early-stopping curve | Section 12.1 | [open early-stopping plot](../result/modeling/operation_length_mins/expected_duration_sensitivity/early_stopping_validation_curve.png) |
| Extra improvement checks | Section 12.2 | [r2_experiments folder](../result/modeling/operation_length_mins/r2_experiments/) |
| Feature importance | Section 12.2 | [latest_best_feature_importance.csv](../result/modeling/operation_length_mins/r2_experiments/latest_best_feature_importance.csv) |
| Benchmark R² plot | Section 12.3 | [open benchmark R² plot](../result/modeling/operation_length_mins/presentation_plots/operation_length_model_benchmark_r2.png) |
| Benchmark MAE plot | Section 12.3 | [open benchmark MAE plot](../result/modeling/operation_length_mins/presentation_plots/operation_length_model_benchmark_mae.png) |
| Test accuracy-style plot | Section 12.3 | [open within-minutes accuracy plot](../result/modeling/operation_length_mins/presentation_plots/operation_length_test_within_minutes_accuracy.png) |
| Feature-importance plot | Section 12.3 | [open feature-importance plot](../result/modeling/operation_length_mins/presentation_plots/operation_length_feature_importance.png) |
| One-page benchmark dashboard | Section 12.3 | [open benchmark dashboard](../result/modeling/operation_length_mins/presentation_plots/operation_length_benchmark_summary_dashboard.png) |
"""),
        code("""
from IPython.display import Image, display

SUPPLEMENT_DIR = PROJECT_ROOT / "result" / "modeling" / "operation_length_mins"
R2_DIR = SUPPLEMENT_DIR / "r2_experiments"
EXPECTED_DURATION_DIR = SUPPLEMENT_DIR / "expected_duration_sensitivity"

def read_optional_csv(path):
    if path.exists():
        return pd.read_csv(path)
    print(f"Missing optional result file: {path.relative_to(PROJECT_ROOT)}")
    return pd.DataFrame()
...
def show_optional_image(path):
    if path.exists():
        ...
        print(f"Missing optional figure: {path.relative_to(PROJECT_ROOT)}")

result_reference_table = pd.DataFrame([
    {
        "evidence": "Main algorithm benchmark",
        "notebook section": "Section 7",
        "open result": '<a href="../result/modeling/operation_length_mins/algorithm_comparison.csv" target="_blank">algorithm_comparison.csv</a>',
    },
    {
        "evidence": "Dummy and hospital-duration benchmark",
        "notebook section": "Section 10",
        "open result": '<a href="../result/modeling/operation_length_mins/operational_benchmarks.csv" target="_blank">operational_benchmarks.csv</a>',
    },
    {
        "evidence": "Final untouched-test metrics",
        "notebook section": "Section 9",
        "open result": '<a href="../result/modeling/operation_length_mins/final_test_metrics.csv" target="_blank">final_test_metrics.csv</a>',
    },
    {
        "evidence": "ExpectedDurationMins with/without + early stopping",
        "notebook section": "Section 12.1",
        "open result": '<a href="../result/modeling/operation_length_mins/expected_duration_sensitivity/expected_duration_sensitivity_results.csv" target="_blank">expected_duration_sensitivity_results.csv</a>',
    },
    {
        "evidence": "Validation versus test plot",
        "notebook section": "Section 12.1",
        "open result": '<a href="../result/modeling/operation_length_mins/expected_duration_sensitivity/validation_test_comparison.png" target="_blank">open validation/test plot</a>',
    },
    {
        "evidence": "Early-stopping validation curve",
        "notebook section": "Section 12.1",
        "open result": '<a href="../result/modeling/operation_length_mins/expected_duration_sensitivity/early_stopping_validation_curve.png" target="_blank">open early-stopping plot</a>',
    },
    {
        "evidence": "Feature importance for latest best setup",
        "notebook section": "Section 12.2",
        "open result": '<a href="../result/modeling/operation_length_mins/r2_experiments/latest_best_feature_importance.csv" target="_blank">latest_best_feature_importance.csv</a>',
    },
])
from IPython.display import HTML
HTML(result_reference_table.to_html(escape=False, index=False))
"""),
        md("""
### 12.1 ExpectedDurationMins sensitivity with early stopping

The supervisor suggested checking whether the hospital planned duration should be removed. We tested the same model with and without `ExpectedDurationMins`, using a grouped train/validation/test split and XGBoost early stopping. The outcome is regression, so the key scores are R² and MAE, not classification accuracy.

[Click here to open the validation/test plot](../result/modeling/operation_length_mins/expected_duration_sensitivity/validation_test_comparison.png)

[Click here to open the learning / early-stopping plot](../result/modeling/operation_length_mins/expected_duration_sensitivity/early_stopping_validation_curve.png)
"""),
        code("""
expected_duration_results = read_optional_csv(
    EXPECTED_DURATION_DIR / "expected_duration_sensitivity_results.csv"
)
if not expected_duration_results.empty:
    display(expected_duration_results[[
        "experiment", "predictors", "best iteration",
        "validation MAE", "validation R2", "test MAE", "test R2",
    ]])
    with_expected = expected_duration_results.set_index("experiment").loc["With ExpectedDurationMins"]
    without_expected = expected_duration_results.set_index("experiment").loc["Without ExpectedDurationMins"]
    print(
        "Removing ExpectedDurationMins changed test R2 from "
        f"{with_expected['test R2']:.3f} to {without_expected['test R2']:.3f} "
        "and test MAE from "
        f"{with_expected['test MAE']:.2f} to {without_expected['test MAE']:.2f} minutes."
    )
"""),
        code("""
show_optional_image(EXPECTED_DURATION_DIR / "validation_test_comparison.png")
show_optional_image(EXPECTED_DURATION_DIR / "early_stopping_validation_curve.png")
"""),
        md("""
**Interpretation.** Removing `ExpectedDurationMins` made the model worse. This is expected because the planned duration is the hospital's pre-operative estimate of case length. It is not a leakage column for direct operation-length prediction because it is available before the operation. It remains a required planning predictor and is not imputed when missing.
"""),
        md("""
...
"""),
        code("""
optional_tables = {
    "K-fold number check": R2_DIR / "kfold_number_experiments.csv",
    "Consultant sensitivity": R2_DIR / "consultant_sensitivity_experiments.csv",
    "Full procedure code sensitivity": R2_DIR / "full_procedure_code_sensitivity_experiments.csv",
    "Procedure chapter sensitivity": R2_DIR / "procedure_chapter_sensitivity_experiments.csv",
    "Best columns, chapter, normalization and skew": R2_DIR / "best_columns_chapter_normalized_skew_experiments.csv",
    "Trim rare-grouped full code": R2_DIR / "trim_full_code_min20_experiment.csv",
    "Trimmed start-hour impact": R2_DIR / "trimmed_columns_start_hour_impact.csv",
    "Neural network sensitivity": R2_DIR / "neural_network_no_priority_best_columns.csv",
    "CatBoost comparison": R2_DIR / "catboost_operation_length_experiments.csv",
}
summary_rows = []
for label, path in optional_tables.items():
    table = read_optional_csv(path)
    if table.empty:
        continue
    display_columns = [c for c in [
        "experiment", "model", "target", "predictors", "folds",
        "validation MAE", "validation R2", "CV MAE", "CV R2",
        "test MAE", "test R2", "MAE", "R2",
    ] if c in table.columns]
    print(f"\\n{label}")
    display(table[display_columns].head(10))
    score_column = "test R2" if "test R2" in table.columns else "R2" if "R2" in table.columns else None
    mae_column = "test MAE" if "test MAE" in table.columns else "MAE" if "MAE" in table.columns else None
    if score_column:
        best = table.sort_values(score_column, ascending=False).iloc[0]
        summary_rows.append({
            "check": label,
            "best option": best.get("experiment", best.get("model", "not labelled")),
            "R2": best.get(score_column),
            "MAE": best.get(mae_column) if mae_column else np.nan,
            "source": str(path.relative_to(PROJECT_ROOT)),
        })
extra_experiment_summary = pd.DataFrame(summary_rows)
extra_experiment_summary
"""),
        code("""
feature_importance_extra = read_optional_csv(R2_DIR / "latest_best_feature_importance.csv")
if not feature_importance_extra.empty:
    display(feature_importance_extra[[
        "rank", "feature", "MAE increase when shuffled", "importance SD"
    ]])
    fig, ax = plt.subplots(figsize=(9, 5))
    sns.barplot(
        data=feature_importance_extra.sort_values("MAE increase when shuffled", ascending=True),
        x="MAE increase when shuffled", y="feature", color="#59A14F", ax=ax,
    )
    ax.set(
        title="Supplementary permutation importance: operation_length_mins",
        xlabel="MAE increase after shuffling, minutes",
        ylabel="",
    )
    plt.tight_layout()
"""),
...
These plots are designed for the supervisor presentation. For this regression target, “accuracy” is shown in two ways: R², where higher is better, and within-minutes accuracy, which shows the percentage of test cases predicted within 10, 20, 30 and 60 minutes.

[Click here to open the one-page benchmark dashboard](../result/modeling/operation_length_mins/presentation_plots/operation_length_benchmark_summary_dashboard.png)

[Click here to open the benchmark R² plot](../result/modeling/operation_length_mins/presentation_plots/operation_length_model_benchmark_r2.png)

[Click here to open the benchmark MAE plot](../result/modeling/operation_length_mins/presentation_plots/operation_length_model_benchmark_mae.png)

[Click here to open the test within-minutes accuracy plot](../result/modeling/operation_length_mins/presentation_plots/operation_length_test_within_minutes_accuracy.png)

[Click here to open the feature-importance plot](../result/modeling/operation_length_mins/presentation_plots/operation_length_feature_importance.png)
"""),
        code("""
PRESENTATION_PLOT_DIR = SUPPLEMENT_DIR / "presentation_plots"
show_optional_image(PRESENTATION_PLOT_DIR / "operation_length_benchmark_summary_dashboard.png")
show_optional_image(PRESENTATION_PLOT_DIR / "operation_length_model_benchmark_r2.png")
show_optional_image(PRESENTATION_PLOT_DIR / "operation_length_model_benchmark_mae.png")
show_optional_image(PRESENTATION_PLOT_DIR / "operation_length_test_within_minutes_accuracy.png")
show_optional_image(PRESENTATION_PLOT_DIR / "operation_length_feature_importance.png")
"""),
    ]
...
The modelling order is deliberately leakage-safe:

1. Build the target-eligible population and prohibit outcome leakage.
2. Define predictors `X` and target `y`.
3. Freeze grouped development/test partitions before fitting any preprocessing step.
4. Inside each development cross-validation fold, fit preprocessing on the training part only:
   - numeric missing values: median imputation plus missingness indicators;
   - categorical missing values: explicit `Missing/not recorded` category;
   - categorical encoding: one-hot encoding with rare-category handling;
   - numeric scaling: `StandardScaler`.
5. Use an interpretable baseline model to compare missing-data configurations.
6. Freeze the missing strategy and compare feature configurations.
7. Freeze the data configuration and compare six algorithms, including a neural network, plus a dummy benchmark.
8. Tune the two leading algorithms using development folds only.
9. Freeze the model and evaluate the untouched test set once.
10. Run start-hour, flagged-record and complete-case sensitivity analyses.

Skewed-target treatment, when tested, is also fitted inside the modelling experiment rather than applied permanently to the cleaned dataset. This avoids leaking validation or test information into training.
...
The missing strategy, predictor columns and folds are now frozen. All six algorithms — including a regularized multilayer neural networkâ€”and the dummy benchmark receive identical development information. The neural network uses early stopping within each training fit.
"""),
        code("""
...
"""),
        *(operation_length_supplementary_cells() if target == "operation_length_mins" else []),
        md("""
